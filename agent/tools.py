...
async def read_file(args: ReadFileArgs, state: RunState, context: AgentContext) -> dict[str, Any]:
    path = Path(args.path)
...
async def modify_todo(args: ModifyTodoArgs, state: RunState, context: AgentContext) -> dict[str, Any]:
    if args.action == "add":
...
{chr(10).join(state.todos)}
    ...
    if args.action == "remove":
        ...
        if missing:
            return {
                "error": f"Todos {', '.join(missing)} not found",
            }
...
MODIFY_TODO_TOOL = Tool(
    name="modify_todo",
    description="Modify the todos list. Add or remove todos from the list.",
